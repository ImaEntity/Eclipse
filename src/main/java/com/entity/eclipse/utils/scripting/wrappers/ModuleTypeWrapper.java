package com.entity.eclipse.utils.scripting.wrappers;

import com.entity.eclipse.modules.ModuleType;

public class ModuleTypeWrapper {
    public final ModuleType COMBAT = ModuleType.COMBAT;
    public final ModuleType MOVEMENT = ModuleType.MOVEMENT;
    public final ModuleType PLAYER = ModuleType.PLAYER;
    public final ModuleType RENDER = ModuleType.RENDER;
    public final ModuleType WORLD = ModuleType.WORLD;
    public final ModuleType EXPLOIT = ModuleType.EXPLOIT;
    public final ModuleType MISC = ModuleType.MISC;
}
